...
library(rgdal)               #spTransform from lat-long to UTM
library(maptools)            # to import/export shapefiles
library(readxl)              # 2019-07-14 
library(ggplot2)
library(amt)
library(moveHMM)            # see also momentuHMM
library(sjPlot)             # https://dmyee.files.wordpress.com/2016/03/table_workshop.pdf
library(ggsci)
...
# Confidence intervals Now that you are convinced you have a defensible HMM, you can also get confidence intervals for your parameter estimates
#CI(m)
...
tab_df(m.move.tidy,
       ...
       digits = 3,
       file = "Table S1. Movement Model Parameters.doc")
...
ggsave('Fig. S1. State probability vs night.png', width = 6, height = 5, scale = 0.9)
